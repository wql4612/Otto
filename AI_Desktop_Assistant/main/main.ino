 ...
 * 语音：Edge Impulse 本地唤醒词 + 云端 DashScope ASR/LLM
...
// ── 云桥接配置 ──
#define SERVER_IP "10.138.77.104" // ← 改成你电脑的局域网 IP
#define SERVER_PORT 8765
#define DIALOG_TIMEOUT_MS 15000 // 对话超时（毫秒）
...
static DialogState g_dialog = DIALOG_IDLE;
static unsigned long g_dialog_timeout = 0;

// ── 音频流任务 ──
static TaskHandle_t g_task_audio_stream = nullptr;
...
{
    if (total == 0 || total > IMAGE_BYTES || !g_image_buf)
        return;
    if (index + len > total)
        len = total - index;
...
static VoiceCommand parse_server_cmd(const String &json)
{
    int i = json.indexOf("\"command\":\"");
    if (i < 0)
        return CMD_NONE;
    i += 12;
    ...
    if (s == "CMD_HELLO")
        return CMD_HELLO;
    if (s == "CMD_COME_HERE")
        return CMD_COME_HERE;
    if (s == "CMD_RELAX_MODE")
        return CMD_RELAX_MODE;
    if (s == "CMD_WORK_MODE")
        return CMD_WORK_MODE;
    if (s == "CMD_WHAT_TIME")
        return CMD_WHAT_TIME;
    if (s == "CMD_HOME_MODE")
        return CMD_HOME_MODE;
    if (s == "CMD_AWAY_MODE")
        return CMD_AWAY_MODE;
    return CMD_NONE;
}

static void on_server_text(const String &json)
{
    Serial.printf("[Cloud] %s\n", json.c_str());
    // 指令
    if (json.indexOf("\"type\":\"cmd\"") >= 0)
    {
        VoiceCommand cmd = parse_server_cmd(json);
        if (cmd != CMD_NONE)
        {
            voice_set_command(cmd);
            log_event("command", voice_command_name(cmd));
        }
        return;
    }
    // AI 文本
    if (json.indexOf("\"type\":\"chat\"") >= 0)
    {
        ...
        if (e < 0)
            return;
        set_last_result(json.substring(i, e));
        g_dialog_timeout = millis() + DIALOG_TIMEOUT_MS;
        return;
    }
    // 增量文本 (刷新超时)
    if (json.indexOf("\"type\":\"chat_partial\"") >= 0)
    {
        g_dialog_timeout = millis() + DIALOG_TIMEOUT_MS;
        return;
    }
    // TTS 开始
    if (json.indexOf("\"type\":\"tts_start\"") >= 0)
    {
        g_tts_playing = true;
        g_dialog = DIALOG_PLAYING;
        return;
    }
    // TTS 结束 → 继续 ASR
    if (json.indexOf("\"type\":\"audio_done\"") >= 0)
    {
        g_tts_playing = false;
        g_dialog = DIALOG_ACTIVE;
        g_dialog_timeout = millis() + DIALOG_TIMEOUT_MS;
        cloud_bridge_send_text("START");
        return;
    }
}

// TTS 音频回调 (24kHz mono → stereo)
static void on_server_binary(const uint8_t *data, size_t len)
{
    size_t n = len / 2;
    if (n == 0)
        return;
    int16_t *stereo = (int16_t *)ps_malloc(n * 2 * sizeof(int16_t));
    if (!stereo)
        return;
    ...
    speaker_play(stereo, n * 2, true);
    free(stereo);
}

// 音频流发送任务
static void task_audio_stream_fn(void *pv)
{
    (void)pv;
    const size_t chunk = 640; // 20ms @ 16kHz
    uint8_t *buf = (uint8_t *)malloc(chunk);
    if (!buf)
    {
        g_task_audio_stream = nullptr;
        vTaskDelete(NULL);
        return;
    }
    while (g_streaming_active)
    {
        if (!g_tts_playing && cloud_bridge_connected())
        {
            size_t got = mic_record(buf, chunk, NULL);
            if (got > 0)
                cloud_bridge_send_binary(buf, got);
        }
...
static void dialog_enter()
{
    g_dialog = DIALOG_ACTIVE;
    g_dialog_timeout = millis() + DIALOG_TIMEOUT_MS;
    g_current_face = FACE_LISTENING;
    screen_show_face_jpeg(FACE_FILES[FACE_LISTENING]);
    play_tone(880, 80, 16000);
    g_streaming_active = true;
    xTaskCreatePinnedToCore(task_audio_stream_fn, "audio_str", 4096, NULL, 2, &g_task_audio_stream, 0);
    cloud_bridge_send_text("START");
    log_event("voice", "Wake → dialog");
}

static void dialog_exit()
{
    g_streaming_active = false;
    g_tts_playing = false;
    cloud_bridge_send_text("STOP");
    g_dialog = DIALOG_IDLE;
    g_current_face = FACE_IDLE;
    screen_show_face_jpeg(FACE_FILES[FACE_IDLE]);
    log_event("voice", "Dialog timeout");
}
...
void setup()
{
    ...
    // 3. 麦克风
    if (!mic_init())
        set_last_result("Mic: FAIL");
    else
        set_last_result("Mic: OK");

    // 4. 扬声器 (24kHz 以匹配 TTS 音频)
    if (!speaker_init(6, 5, 1, 24000))
        set_last_result("Speaker: FAIL");
    else
    {
        set_last_result("Speaker: OK (24kHz)");
        play_tone(880, 80, 16000);
    }
    ...
    // 6. 摄像头
    if (!camera_init())
        set_last_result("Camera: FAIL");
    else
        set_last_result("Camera: OK");
    ...
    // 9. LittleFS
    if (!wifi_littlefs_init())
        set_last_result("LittleFS: FAIL");
    else
        set_last_result("LittleFS: OK");
    ...
    // 11. 云桥接（连接 Python 服务器）
    cloud_bridge_on_text(on_server_text);
    cloud_bridge_on_binary(on_server_binary);
    if (cloud_bridge_init(SERVER_IP, SERVER_PORT))
    {
        set_last_result("Cloud: Connected");
    }
    else
    {
        set_last_result("Cloud: FAIL — server not reachable");
    }
...
void loop()
{
    ...
    // ── 8. 本地 KWS 唤醒词检测 (仅在 IDLE 时运行) ──
    if (g_dialog == DIALOG_IDLE)
    {
        voice_loop();
        if (voice_wake_detected())
        {
            dialog_enter();
        }
    }
    ...
    // ── 10. 云桥接自动重连 ──
    static unsigned long last_cloud_retry = 0;
    if (!cloud_bridge_connected() && millis() - last_cloud_retry > 5000)
    {
        last_cloud_retry = millis();
        cloud_bridge_init(SERVER_IP, SERVER_PORT);
    }

    // ── 11. 对话超时 ──
    if (g_dialog != DIALOG_IDLE && millis() > g_dialog_timeout)
    {
        dialog_exit();
    }
