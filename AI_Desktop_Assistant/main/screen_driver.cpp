...
bool screen_init(uint8_t cs_pin, uint8_t dc_pin, int8_t rst_pin,
    ...
    if (!tft) {
        tft = new Adafruit_ST7789(cs_pin, dc_pin, rst_pin);
    }
    if (!tft) {
...
void screen_show_message(const char* line1, const char* line2, const char* line3) {
    ...
    screen_clear(ST77XX_BLACK);
    draw_line(line1, 18, 44, ST77XX_WHITE, 3);
    draw_line(line2, 18, 104, ST77XX_YELLOW, 2);
    draw_line(line3, 18, 146, ST77XX_CYAN, 2);
